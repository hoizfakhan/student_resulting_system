...
export default function Create({
  ...
  const initialMarks = students.data.map(student => {
    const mark = student.marks && student.marks.length > 0 ? student.marks[0] : { chance:'',home_work: '', class_activity: '',midterm:'',final:'' };
    return {
  ...
  const handleSaveMarks = (studentId) => {
    ...
    const marksData = {
      ...
      chance: parseInt(studentMarks.chance || 0),
  ...
  const handleSaveAllMarks = () => {
    ...
    post(route("marks.storeAll", [data.subject_id]), {
      marks: marksData,
    });
  };
            ...
            <div className="overflow-auto">
              ...
              {students.data && students.data.length > 0 ? (
                <form onSubmit={onSubmit}>
                      ...
                      {students.data.map((student) => (
                        <tr key={student.id}>
                          ...
                          <td className="px-3 py-2">
                            {/* Select input for chance */}
                            <SelectInput
                              className="form-control"
                              id={`chance_${student.id}`}
                              name={`chance_${student.id}`}
                              value={data.marks.find(
                                (m) => m.student_id === student.id
                              )?.chance || ""}
                              onChange={(e) =>
                                handleArrayChange(e, student.id, "chance")
                              }
                            >
                          ...
                          <td className="px-3 py-2">
                             {/* Button to save marks for the student */}
                                <button
                  ...
                  <button
                    className="btn btn text-center btn-outline-success"
